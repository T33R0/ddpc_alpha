...
export default function VehicleWishlistPage({ params }: { params: { id: string } }) {
  const vehicleId = params.id;
  const [items, setItems] = useState<WishlistItem[]>([]);
  const [loading, setLoading] = useState(false);
  const [boardMode, setBoardMode] = useState(false); // Budget Board sub-tab
  ...
  useEffect(() => {
    // Initial fetch (placeholder - wire later to GET /api/wishlist?vehicleId=...)
    setItems([]);
  }, [vehicleId]);
  ...
  function addToBudget(id: string) {
    // Could navigate to dedicated budget UI; for now, stub
  ...
  return (
    <div className="space-y-6">
      ...
      {!boardMode && (
        <>
          {items.length === 0 ? (
            ...
            <div className="columns-1 sm:columns-2 lg:columns-3 gap-4 [column-fill:_balance]">{/* Masonry via CSS columns */}
              {items.map((it) => (
      ...
      <div className="pt-6 border-t text-xs text-muted">
        <Link href={`/vehicles/${vehicleId}`}>Back to Specs</Link>
      </div>
    </div>
  );
}
