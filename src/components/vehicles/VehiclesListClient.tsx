...
export default function VehiclesListClient({
  ...
  const [covers, setCovers] = useState<Record<string, string | null>>(() => {
    ...
    for (const v of vehicles) {
      initial[v.id] = v.photo_url && v.photo_url.trim().length > 0 ? v.photo_url : null;
    }
    return initial;
  });
  ...
  const filtered: VehicleRow[] = useMemo(() => {
    ...
    const list: VehicleRow[] = ql
      ...
      : vehicles.slice();
    const lastUpdatedMs = (v: VehicleRow) => v.last_event_at ? Date.parse(v.last_event_at) : v.updated_at ? Date.parse(v.updated_at) : v.created_at ? Date.parse(v.created_at) : 0;
    ...
    <div className="space-y-4">
      <div className="flex items-end gap-3">
        ...
        <div>
          ...
          <select
            value={sortKey}
            onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setSortKey(e.target.value as "updated" | "name" | "year")}
