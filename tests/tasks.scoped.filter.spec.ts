...
test.describe.configure({ tag: ['@auth', '@tasks', '@scoped'] });

test('VehicleFilter defaults to current id and changing navigates tasks', async ({ page }) => {
  ...
  // Navigate to tasks for A
  await goto(page, `/vehicles/${vehicleA}/tasks`);
  ...
  await expect(page).toHaveURL(new RegExp(`/vehicles/${valueB}/tasks$`));
